---
hide:
  - navigation
  - toc
---

# Model Comparison & Evaluation

Compare multiple models side-by-side using the Evaluator API with custom cross-validation strategies, per-fold metric breakdowns, lift analysis, and calibration.

<div class="iframe-container" id="iframe-wrapper-model-comparison">
  <div class="iframe-controls">
    <button type="button" class="md-button notebook-expand-btn">Expand</button>
    <a href="/iter8ml/notebooks/html/03_model_comparison.html" target="_blank" rel="noopener noreferrer" class="md-button">Open in New Tab</a>
  </div>
  <iframe src="/iter8ml/notebooks/html/03_model_comparison.html" allowfullscreen loading="lazy"></iframe>
</div>

## Run Locally

```bash
uv run quarto preview notebooks/03_model_comparison.qmd
```
