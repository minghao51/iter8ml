...
### 2. MCP Layer (`src/iter8ml/services/mcp.py`)
- FastMCP server (`iter8ml`) exposing atomic tools for LLM agents
...
### 5. Engine Layer (`src/iter8ml/engine/`)
...
### 6. Pipeline Layer (`src/iter8ml/pipelines/`)
...
### 7. Pipeline Nodes (`src/iter8ml/pipelines/nodes/`)
...
### 8. Models Layer (`src/iter8ml/models/`)
...
### 9. Monitoring Layer (`src/iter8ml/monitoring/`)
...
### 10. Services Layer (`src/iter8ml/services/`)
...
|---|---|---|
| CLI | `iter8` console script | `pyproject.toml:59` -> `cli.py:18` |
| MCP Server | FastMCP | `mcp/tools.py:17` |
| Direct Python | `from iter8ml import ...` | `__init__.py` |

## Key Files

| File | Purpose |
|---|---|
| `src/iter8ml/cli.py` | Typer CLI entry point with 10 commands |
| `src/iter8ml/config.py` | ExperimentConfig + HardwareProfile |
| `src/iter8ml/constants.py` | Enums and conversion utilities |
| `src/iter8ml/exceptions.py` | Exception hierarchy + track_errors decorator |
| `src/iter8ml/engine/trainer.py` | Main orchestrator |
| `src/iter8ml/pipelines/executor.py` | Hamilton driver builder per mode |
| `src/iter8ml/engine/tracker.py` | Tracker protocol + implementations |
| `src/iter8ml/models/base.py` | AbstractModel Protocol |
| `src/iter8ml/models/factory.py` | Model class registry |
| `src/iter8ml/services/registry_service.py` | File-locked champion registry |
| `src/iter8ml/utils/safe_pickle.py` | Restricted unpickler |
