...
def null_filled_df(
    ...
    numeric_filled = fill_nulls_numeric.select([pl.col(c) for c in numeric_columns])
    categorical_filled = fill_nulls_categorical.select([pl.col(c) for c in categorical_columns])
    other_cols = [
        c
        for c in fill_nulls_numeric.columns
        if c not in numeric_columns and c not in categorical_columns
    ]
    other_df = fill_nulls_numeric.select(other_cols) if other_cols else pl.DataFrame()
    parts = [p for p in [other_df, numeric_filled, categorical_filled] if p.width > 0]
    if not parts:
        return fill_nulls_numeric
